...
class HaikuGenerator:
    """Main class responsible for haiku generation"""

    def __init__(self, degree=2):
        """Constructor for the class. Initializes the attempt counter, number
        of maximum attempts, line indicator, the degree (Markov order) used in
        generation, an empty Trie structure and the syllable limits for different lines

        Args:
            degree: number, order of the Markov chain used
        """
        self._attempt = 1
        self._degree = degree
        self._line = 1
        self._max_attempts = MAX_ATTEMPTS
        self._syllable_limits = {1: 5, 2: 7, 3: 5}
    ...
    def attempt_haiku_generation(self):
        ...
        while self._attempt <= self._max_attempts:
            ...
            if self._is_valid_haiku(haiku):
                return self._get_formatted_haiku(haiku)
            self._attempt += 1
        raise Exception(
    ...
    def _generate_line(self, segment):
        ...
        syllable_limit = self._syllable_limits[self._line]
        ...
        while remaining_syllables > 0:
            ...
            try:
                token = self._generate_word(node, remaining_syllables)
                if token.isalpha() and remaining_syllables != self._syllable_limits[self._line]:
                    line += " "
                line += token
    ...
    def _generate_word(self, node, remaining_syllables):
        ...
        if valid_choices:
            token = Node.get_random_node(valid_choices.values()).value
            return token
        raise Exception(
    ...
    def _is_vowel(self, phone):
        ...
        In CMU dictionary vowel phomenes include a stress marker, which is a number.
    ...
    def _is_valid_token(self, token, remaining_syllables):
        ...
        syllables = self._syllable_limits[self._line]
    ...
    def _is_valid_haiku(self, haiku):
        ...
        for index, line in enumerate(haiku_lines, start=1):
            ...
            if syllable_count != self._syllable_limits[index]:
                return False
        return True

    def _get_formatted_haiku(self, haiku):
